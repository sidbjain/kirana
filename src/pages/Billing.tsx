...
export function Billing() {
    ...
    const updateQuantity = (id: string, newQty: number) => {
        ...
        setCart((prev) =>
            prev.map((item) => {
                if (item.id === id) {
                    // Allow detailed decimal updates
    ...
    const updateAmount = (id: string, newAmount: number) => {
        ...
        setCart((prev) =>
            prev.map((item) => {
                if (item.id === id) {
                    // Reverse calculate quantity: Qty = Amount / Price
    ...
    const handlePrint = () => {
        ...
        cart.forEach(item => {
            ...
            if (product) {
                const newStock = Math.max(0, (product.stock || 0) - item.quantity); // Prevent negative stock
        ...
        // Optional: Clear cart after printing (commented out for now to allow reprint if needed)
        // setCart([]);
    }

    return (
        <div className="bg-gray-50 h-[calc(100vh-2rem)] flex gap-6">
                ...
                <div className="flex-1 overflow-y-auto space-y-3 pr-2">
                    {filteredProducts.map(product => (
                        <div
                            ...
                            className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 cursor-pointer hover:border-orange-500 transition-all flex justify-between items-center group"
                        >
                            <div>
                                <h3 className="font-bold text-gray-800">{product.name}</h3>
                                <p className="text-sm text-gray-500">₹{product.price} / {product.unit}</p>
                            </div>
                            <div className="h-8 w-8 rounded-full bg-orange-50 flex items-center justify-center text-orange-600 opacity-0 group-hover:opacity-100 transition-opacity">
                                <Plus size={18} />
                            </div>
                        </div>
                    ))}
            ...
            <div className="w-1/2 bg-white rounded-xl shadow-sm border border-gray-200 flex flex-col overflow-hidden h-full">
                <div id="printable-bill" className="flex-1 flex flex-col p-6 overflow-y-auto">
                                            ...
                                            <div className="flex flex-col items-center">
                                                <div className="flex items-center justify-center gap-2 print:hidden">
                                                    ...
                                                    <input
                                                        ...
                                                        className="w-16 text-center border rounded py-1 text-sm outline-none focus:border-orange-500"
                                                    />
                                                    <button
                                                        onClick={() => updateQuantity(item.id, item.quantity + 1)}
                                                        className="p-1 hover:bg-gray-100 rounded text-gray-500"
                                        ...
                                        <td className="py-3 text-right font-medium text-gray-900">
                                            <div className="print:hidden">
                                                <input
                                                    ...
                                                    className="w-20 text-right border rounded py-1 text-sm outline-none focus:border-orange-500"
                                                />
                                            </div>
